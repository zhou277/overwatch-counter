...
def tip_card(rank, text, top10):
    ...
    return (
        f'<div class="tip-card {css}">'
        f'<div class="tip-rank">Top {rank}</div>'
...
def main():
    ...
    settings = {
        clean(r["字段"]): clean(r["值"])
        for r in rows_as_dicts(wb["settings"])
    }

    hero_rows = sorted(
        rows_as_dicts(wb["heroes"]),
        key=lambda r: int(r["order"] or 9999)
    )
    ...
    counters = [
        r for r in rows_as_dicts(wb["counters"])
        if clean(r["relation"]).lower() == "weak"
    ]
    ...
    for name in hero_names:
        hero_by_name[name]["weak"] = (
            matchup[name]["weak"]["坦克"]
            + matchup[name]["weak"]["输出"]
            + matchup[name]["weak"]["支援"]
        )

    map_rows = sorted(
        rows_as_dicts(wb["maps"]),
        key=lambda r: int(r["order"] or 9999)
    )
    maps = []
    for r in map_rows:
    ...
    tips = sorted(
        rows_as_dicts(wb["tips"]),
        key=lambda r: int(r["rank"] or 9999)
    )
    ...
    output = output.replace("__VERSION__", settings.get("version", "v3.2.0"))

    OUTPUT.write_text(output, encoding="utf-8")
    print(f"Generated {OUTPUT.name}: {len(heroes)} heroes, {len(counters)} weak-counter rows")
